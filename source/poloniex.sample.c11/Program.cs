using Poloniex.API.User;
using Poloniex.API.Public;
using Poloniex.API.Trade;
using Poloniex.LIB.Types;
using System;
using System.Linq;

namespace Poloniex.Sample.Core
{
    class Program
    {
        /// <summary>
        /// 1. Public API
        /// </summary>
        public static async void XPublicApi(int skip_step = 5)
        {
            var _public_api = new XPublicApi();

            if (skip_step == 1)
            {
                var _ticker = await _public_api.GetTicker();
                if (_ticker.Count > 0)
                    Console.WriteLine(_ticker.Count);
            }

            if (skip_step == 2)
            {
                var _orderbook = await _public_api.GetOrderBook(CurrencyPair.Parse("BTC_XRP"), 20);
                if (_orderbook.SellOrders.Count > 0)
                    Console.WriteLine(_orderbook.SellOrders.Count);
            }

            if (skip_step == 3)
            {
                var _trades = await _public_api.GetTradeHistory(CurrencyPair.Parse("BTC_XRP"));
                if (_trades.Count > 0)
                    Console.WriteLine(_trades.Count);
            }

            if (skip_step == 4)
            {
                var _end_time = DateTime.Now;
                var _start_time = _end_time.AddHours(-10);

                var _trades = await _public_api.GetTradeHistory(CurrencyPair.Parse("BTC_XRP"), _start_time, _end_time);
                if (_trades.Count > 0)
                    Console.WriteLine(_trades.Count);
            }

            if (skip_step == 5)
            {
                var _end_time = DateTime.Now;
                var _start_time = _end_time.AddDays(-10);

                var _trades = await _public_api.GetChartData(CurrencyPair.Parse("BTC_XRP"), ChartPeriod.Minutes30, _start_time, _end_time);
                if (_trades.Count > 0)
                    Console.WriteLine(_trades.Count);
            }
        }

        /// <summary>
        /// 2. User API
        /// </summary>
        public static async void XUserApi(int skip_step = 5)
        {
            var __info_api = new XUserApi("", "");

            if (skip_step == 1)
            {
                var _balances = await __info_api.GetBalances();
                if (_balances.Count > 0)
                    Console.WriteLine(_balances.Where(b => b.Key == "XRP").SingleOrDefault().Value.QuoteAvailable);
            }

            if (skip_step == 2)
            {
                var _addresses = await __info_api.GetDepositAddresses();
                if (_addresses.Count > 0)
                    Console.WriteLine(_addresses.Count);
            }

            if (skip_step == 3)
            {
                var _end_time = DateTime.Now;
                var _start_time = _end_time.AddHours(-10);

                var _deposit_withdrawal = await __info_api.GetDepositsAndWithdrawals(_start_time, _end_time);
                if (_deposit_withdrawal.Deposits.Count > 0 || _deposit_withdrawal.Withdrawals.Count > 0)
                    Console.WriteLine(_deposit_withdrawal.Deposits.Count);
            }

            if (skip_step == 4)
            {
                var _address = await __info_api.GenerateNewDepositAddress("LTC");
                if (_address.IsGenerationSuccessful == true)
                    Console.WriteLine(_address.Address);
            }

            if (skip_step == 5)
            {
                var _withdraw = await __info_api.Withdrawal("XRP", 10.15m, "rsG1sNifXJxGS2nDQ9zHyoe1S5APrtwpjV", "1000000578");
                if (_withdraw.IsGenerationSuccessful == true)
                    Console.WriteLine(_withdraw.Address);
            }
        }

        /// <summary>
        /// 3. Trade API
        /// </summary>
        public static async void XTradeApi(int skip_step = 4)
        {
            var __trade_api = new XTradeApi("", "");

            if (skip_step == 1)
            {
                var _open_orders = await __trade_api.OpenOrders(CurrencyPair.Parse("BTC_XRP"));
                if (_open_orders.Count > 0)
                    Console.WriteLine(_open_orders.Count);
            }

            if (skip_step == 2)
            {
                var _end_time = DateTime.Now;
                var _start_time = _end_time.AddDays(-10);

                var _trades = await __trade_api.GetTrades(CurrencyPair.Parse("BTC_XRP"), _start_time, _end_time);
                if (_trades.Count > 0)
                    Console.WriteLine(_trades.Count);
            }

            if (skip_step == 3)
            {
                var _post_order = await __trade_api.PlaceOrder(CurrencyPair.Parse("BTC_XRP"), OrderType.Sell, 0.00011000m, 1000.0m);
                Console.WriteLine(_post_order);
            }

            if (skip_step == 4)
            {
                var _delete_order = await __trade_api.DeleteOrder(CurrencyPair.Parse("BTC_XRP"), 60123953979);
                Console.WriteLine(_delete_order);
            }
        }

        /// <summary>
        /// 
        /// </summary>
        /// <param name="args"></param>
        static void Main(string[] args)
        {
            var _debug_step = 2;

            // 1. Public API
            if (_debug_step == 1)
                XPublicApi();

            // 2. Private API
            if (_debug_step == 2)
                XUserApi();

            // 3. Trade API
            if (_debug_step == 3)
                XTradeApi();

            Console.WriteLine("hit any key to quit...");
            Console.ReadKey();
        }
    }
}